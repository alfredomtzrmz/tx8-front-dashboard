  ...
  methods: {
    async storeCategory () {
      ...
      try {
        ...
        this.$notify({ group: 'top', type: 'success', title: '¡Guardado exitosamente!', text: 'La categoría ha sido creada' }, 2500)
        this.category = {}
      } catch (e) {
        this.errors = e.response.status === 422 ? e.response.data.errors : {}
        const message = e.response.status === 422 ? 'Por favor, revisa los campos marcados' : 'Intentalo más tarde'

        this.$notify({ group: 'top', type: 'error', title: '¡Error al guardar!', text: message }, 2500)
      } finally {
