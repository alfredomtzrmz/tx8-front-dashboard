    ...
    <div v-show="isOpen" :class="modalResponsiveClass" class="fixed inset-x-0 bottom-0 z-50 p-4">
      ...
      >
        <div
          ...
          class="flex flex-col transition-all transform bg-white rounded-lg max-h-95-screen min-h-56 shadow-full sm:w-full"
          role="dialog"
          aria-modal="true"
        ...
        >
          <div>
            <header v-if="closeButton || title" class="flex items-center justify-between p-4">
              <h4 v-if="title" class="text-gray-900 sm:text-lg">
                {{ title }}
              </h4>
              <button v-if="closeButton" class="inline-flex items-center justify-center w-8 h-8 ml-auto text-gray-400 rounded-full group focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 hover:bg-gray-50" aria-label="close" type="button" @click="$emit('close')">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-400 group-hover:text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
const AVALIABLE_SIZES = ['sm', 'md', 'lg', 'xl']
export default {
