...
export default {
  ...
  computed: {
    ...mapGetters(['getIsSidebarOpen'])
  },
  methods: {
    ...mapActions(['setSidebarOpen']),
    classActiveRoute (route) {
      return this.$route.path.startsWith(route) ? 'text-blue-600 bg-gray-100' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
    },
    classNestedActiveRoute (route) {
      return this.$route.path === route ? 'bg-blue-50 hover:bg-blue-100 text-blue-500' : 'hover:bg-gray-50 text-gray-500'
    },
    classActiveRouteIcon (route) {
      return this.$route.path.startsWith(route) ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-500'
    }
  }
}
</script>
